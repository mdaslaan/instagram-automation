...
Prompt: {data.get('prompt', 'N/A')[:200]}{'...' if len(str(data.get('prompt', ''))) > 200 else ''}"""
    ...
    def _update_image_preview(self, card, image, label_text="Generated Image"):
        ...
        try:
            ...
            if hasattr(image_preview, 'image'):
                delattr(image_preview, 'image')
            ...
            if image:
                ...
                image_preview.image = photo  # Keep a reference
            else:
    ...
    def _generate_content_for_card(self, card):
        ...
        try:
            ...
            # Generate image using the new Gemini model
            try:
                # Create image generation prompt
                image_prompt = self._create_image_generation_prompt(data, image_url)
                
                # Generate image with the model - FIXED: Use generation_config to specify response types
                ...
                if image_response and hasattr(image_response, 'parts'):
                    # Process image from response parts
                    for part in image_response.parts:
                        if hasattr(part, 'mime_type') and 'image' in part.mime_type:
                            ...
                            generated_image_path = self._save_image(generated_image, f"generated_image_{index + 1}")
                            self.root.after(0, lambda img=generated_image: self._update_image_preview(card, img, "Generated Image"))
                            break
                    else:
                        # If no image part found, use fallback
                        ...
                        self.root.after(0, lambda img=placeholder_image: self._update_image_preview(card, img, "Generated Image (Fallback)"))
                else:
                    raise Exception("No valid image response from model")
                    
            except Exception as e:
                print(f"Image generation failed: {e}")
                self.root.after(0, lambda err=str(e): self._update_image_preview(card, None, f"Image generation failed: {err[:50]}..."))
                
                # Try fallback method for image
                try:
                    placeholder_image = self._create_enhanced_placeholder_image(data)
                    generated_image_path = self._save_image(placeholder_image, f"generated_image_{index + 1}")
                    self.root.after(0, lambda img=placeholder_image: self._update_image_preview(card, img, "Generated Image (Fallback)"))
                except Exception as inner_e:
                    print(f"Fallback image creation failed: {inner_e}")
    ...
    def export_data(self):
        ...
        try:
            ...
            csv_data = []
            for i, entry in enumerate(self.generated_data):
